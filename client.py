...
class IBClient():
    ...
    def get_options_chain(self, conid: str, option_chain_params: Dict) -> Dict :
        opt_chain = {}
        put_collection = []
        call_collection = []
        # obtain current mark for the conid.
        # obtain the available months and strikes
        # return the number of strikes requested in the params
        # for each month along with other requested fields
        # for each contract
        #55=symbol(str), 7295=open_price(float), 86=ask_price(float), 70=high(float),
        # 71=low(float), 84=bid_price(float), 31=last_price(float), 87=volume(int)
        quote_fields = [55, 7295, 86, 70, 71, 84, 31, 87]
        snapshot = self.market_data([str(conid)], since=None, fields=quote_fields)
        if '31' in snapshot[0].keys() :
            last_price = float(self.filter_non_numeric(snapshot[0]['31']))
        else : # Retry a second time
            snapshot = self.market_data([str(conid)], since=None, fields=quote_fields)
            if '31' in snapshot[0].keys():
                last_price = float(self.filter_non_numeric(snapshot[0]['31']))
            else :
                return opt_chain
        # look at the number of strikes to be processed
        num_strikes = option_chain_params['strikeCount']
        range_of_strikes = option_chain_params['range']
        symbol = option_chain_params['symbol']
        contract_object_list, sym_search_info = self.symbol_contracts_and_strikes(symbol=symbol, conid=conid)

        # Find the call strike immediately after the last_price and
        # prune the list to only use the range of strikes specified in the parameters
        for contract in contract_object_list :
            contract_month = contract['Month']
            ntm_calls=[]
            ntm_puts=[]
            strike_counter=0
            for strike_float in contract['Call_strikes']:
                if strike_float > last_price :
                    strike_counter += 1
                    ntm_calls.append(strike_float)
                if strike_counter >= num_strikes :
                    break
            contract['Call_strikes'] = ntm_calls
            for strike_value in contract['Call_strikes'] :
                strike_str = str(strike_value)
                # This retrieves the conid of each option, so we can look up price and volume of each option
                option_info = self.search_option_info(conid=conid, contract_month=contract_month, strike_price=strike_str, put_call_flag='C')
                for call_option in option_info :
                    call_collection.append(call_option)

            # Find the put strikes immediately before the last_price
            # prune the list to only use the range of strikes specified in the parameters
            strike_counter = 0
            for strike_float in reversed(contract['Put_strikes']) :
                if strike_float < last_price :
                    strike_counter += 1
                    ntm_puts.append(strike_float)
                if strike_counter >= num_strikes :
                    break
            contract['Put_strikes'] = ntm_puts
            for strike_value in contract['Put_strikes'] :
                strike_str = str(strike_value)
                # This retrieves the conid of each option, so we can look up price and volume of each option
                option_info = self.search_option_info(conid=conid, contract_month=contract_month, strike_price=strike_str, put_call_flag='P')
                for put_option in option_info :
                    put_collection.append(put_option)

        temp_d = {
            'call_options' : call_collection,
            'put_options' : put_collection
        }
        opt_chain[conid] = temp_d
        return opt_chain
    ...
    def market_data_history(self, conid: str, period: str, bar: str, outsideRth: False) -> Dict:
        """
    ...
    def symbol_contracts_and_strikes(self, symbol: str, conid: str) -> Dict:
        """
            Query strikes for Options/Warrants
        """
        sym_content = self.symbol_search(symbol)
        # obtain conid of the underlying contracts and then obtain strike prices of each one
        sym_conid = sym_content[0]['conid']
        section_info = sym_content[0]['sections']
        # 'sections' is a list of dictionaries of information on the symbol
        # We need to search for the option info 'opt'
        contract_months=[]
        for sect in section_info :
            if sect['secType'] == 'OPT' :
                contract_month_list = sect['months'].split(';')
                # only look out two months
                contract_months.append(contract_month_list[0])
                contract_months.append(contract_month_list[1])
                break;

        contract_strikes = []
        for cont_mth in contract_months :
            putcall_strikes = self.search_strikes(conid=sym_conid, contract_month = cont_mth)
            call_strikes = putcall_strikes['call']
            put_strikes = putcall_strikes['put']
            temp_d = {
                "symbol" : symbol,
                "conid" : sym_conid,
                "Month" : cont_mth,
            ...
            contract_strikes.append(temp_d)


        return contract_strikes, sym_content


    def search_option_info(self, conid: str, contract_month: str, strike_price: str, put_call_flag: str) -> Dict:
        """
                    Performs a search for the strikes for a given symbol and month .
                """
    ...
    def search_strikes(self, conid: str, contract_month: str) -> Dict:
        """
                    Performs a search for the strikes for a given symbol and month .
                """
